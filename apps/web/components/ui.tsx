...
export function Panel({
  ...
  return (
    ...
    >
      {(title || actions) && (
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-white/[0.06] px-4.5 py-3.5">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <h2 className="truncate text-[13.5px] font-semibold tracking-tight text-mute-100">
                {title}
              </h2>
              {subtitle && (
      ...
      {toolbar && (
        <div className="flex shrink-0 items-center gap-1.5 border-b border-white/[0.04] bg-white/[0.01] px-4 py-2">
          {toolbar}
        </div>
      )}
...
const badgeStyles = cva(
  "inline-flex items-center gap-1 rounded-lg px-2 py-0.5 text-[10.5px] font-semibold tracking-wide uppercase border",
  {
    variants: {
      tone: {
...
export function EmptyState({ children, icon }: { children: ReactNode; icon?: ReactNode }) {
  return (
    <div className="flex h-full min-h-24 flex-col items-center justify-center gap-2.5 px-6 text-center">
      ...
      <p className="max-w-[36ch] text-[12.5px] leading-relaxed text-mute-500">{children}</p>
    </div>
  );
}
